...
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.show_features') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
                ...
                <tbody>
                    @foreach ($features as $key => $feature)
                        <tr>
                            ...
                            <td>
                                @if (!$feature->image)
                                    <a href="{{ asset('uploads/default_image.png') }}" data-toggle="lightbox"
                                        data-title="{{ $feature->title }}" data-gallery="gallery">
                                        <img class="index_image" src="{{ asset('uploads/default_image.png') }}"
                                            onerror="this.src='{{ asset('uploads/default_image.png') }}'"
                                            alt="default-image">
                                    </a>
                                @else
                                    <a href="{{ $feature->image }}" data-toggle="lightbox"
                                        data-title="{{ $feature->title }}" data-gallery="gallery">
                                        <img class="index_image" src="{{ $feature->image }}"
                                            onerror="this.src='{{ asset('uploads/default_image.png') }}'"
                                            alt="feature-image">
                                    </a>
                                @endif
                            ...
                            <td>{{ formatDate($feature->created_at) == formatDate($feature->updated_at) ? '--' : formatDate($feature->updated_at) }}
                            </td>
                            <td nowrap="nowrap">
